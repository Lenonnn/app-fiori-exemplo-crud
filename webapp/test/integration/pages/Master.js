		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("uname") === oObject.uname;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].uname;
									} else {
										sCurrentId = aItemsNotInTheList[0].uname;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("uname"),
						title: oBindingContext.getProperty("dats")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "Zcdscrud_abap07 (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'Zcdscrud_abap07 (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("dats");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all Zcdscrud_abap07 to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "Zcdscrud_abap07 (" + iExpectedLength + ")"
								}),
								success: function () {
